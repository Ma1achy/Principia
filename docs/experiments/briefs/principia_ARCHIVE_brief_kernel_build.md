> # ARCHIVED — COMPLETE — built as prin-rs; DEFAULTS HAVE CHANGED
>
> The kernel was built. **Its integrator and event defaults are superseded:**
>
> - **Regularisation: Heggie, not Aarseth–Zare.** 31 of 32 cases, `err>10` 3916 → 73. AZ retains one
>   win — `far`, where sustained hierarchy means it never re-registers. Regularisation is a **swappable
>   axis** independent of the stepper: `principia_integrator_contract.md` Part 2b.
> - **Escape: closure + energy**, not `E>0 ∧ receding ∧ d>r_esc`. The old test fired on transients —
>   **0 of 895 still unbound eight boundaries later.**
> - **Step control:** a **predictive step limit** (which is what removed the wedge artefacts), per-step
>   `dtau`, and a landing clamp. The `dtau` fix alone removed the NaN and left the wedges untouched.
>
> **§5's acceptance gates still stand** and are still the regression set.
>
...
> **The smoke-test number is not a Rust target.** `burrau_grid` draws jitter from one global PCG64
> stream, so per-pixel seeding (§7) cannot reproduce it. The resolution is in the reference: **copy 0
> of every cell is the un-jittered nominal**, so cross-check on *nominal copies only* and no RNG
> participates on either side. Also note the jitter uses the **x** cell width for both axes — correct
> only on square grids; fix per-axis in the port.
...
- Matches the Python reference at f64, reported as a **divergence-vs-horizon table** (`t in {0.5,1,2,4,8,13}`) rather than one pass/fail: a correct port shows divergence growing like `e^{lambda t}` from an `O(1e-16)` intercept. Hard-assert `<=1e-13` at `t<=2` (no amplification, nowhere to hide) and `<=1e-10` at `t=13`. Two f64 implementations differ at ~1 ulp/op, and with `lambda ~ 0.7` a `1e-16` seed reaches ~`1e-12` by `t=13`, so the flat 1e-10 gate is marginal **for reasons unrelated to correctness**
