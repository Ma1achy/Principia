> # ARCHIVED — COMPLETE — built, then superseded in part
>
> The scheduler was built. **The split/stop rules it specifies were then replaced**; the descent,
> decision variants and dump machinery survive.
>
> **Current design:** `principia_dd_refinement_policy.md`. Notably added since: `Decision::Merged` and
> `QuadTree::resident` for the live playhead, the two-level exponent from the grandparent quadrant, and
> neighbour-agreement noise rejection.
>
...
### 2.1 What is measured, per pixel

`E+1` copies at **fixed Halton (2,3) offsets** indexed by `copy_index`, scaled to the cell width.
Fixed, not per-pixel: copy *k* sits at the same relative offset in every quad at every level. This
is what makes parent and child comparable — **common random numbers by construction**.
...
`ensemble_spread` is per **footprint**; `QuadReduction` needs one number per **quad**. With excess
kurtosis 110, **a mean is dominated by a single footprint.** Check what the design docs specify; if
unspecified, dump mean, median and p90 and report which the decisions are sensitive to. **Do not
silently pick one.**
...
A cap on **total quads computed**. When exhausted, stop and report what was left queued. Order the
queue by priority (spread, or spread × area) and **report whether the order mattered** — run the same
budget with a shuffled queue and compare the trees.
