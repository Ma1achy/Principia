...
Intent ──constructs──▶ Chart ──validate──▶ resolve() [Deployment ring]
      ...
      per quad:  (s,t) ─Φ─▶ z ─decode─▶ canonicalise ─▶ (m,r,p)
                                                │
                     wrapper( occupant STEP ×N_sub ) · project · monitor · detect
                                                ▼
...
At `N=8, E+1=8` a quad is **512 trajectories × 136 B = 68 KB**. Workgroup storage limits:
...
**Nothing fits anywhere** — 4.2× over on WebGPU, 2.1× on Metal — and the two backends bind for
...
integration). Provenance → `principia_dd_image_embedding.md` (LSB payload, tile grid, the three
