...
**There is no separate "samples per pixel" dial** — and this is the point that keeps the resolution system coherent. Sample density is **not** an independent quality knob layered on top; it is *fully determined* by the quadtree's screen-space floor. The quadtree subdivides a quad until its tiles reach pixel size (the **screen-space floor**, scheduler contract), and *that is the resolution mechanism*. So at the refinement steady state, **there is exactly one real sample per render pixel, by construction** — you cannot hold "half a sample per pixel" as a setting, because the floor keeps subdividing until tiles equal pixels. An earlier draft had an `spp` ("samples per rendered pixel") tier knob; it was **deleted** because it directly contradicts the screen floor (sub-1 spp fights the floor that drives density to 1) and duplicates E (super-1 spp *is* the ensemble). The three real levers are `render_scale` (how many pixels you render), the refinement floor (how far quads subdivide — normally to pixel size), and E (ensemble/SSAA, the only supersampling path).
