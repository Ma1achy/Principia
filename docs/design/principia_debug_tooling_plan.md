...
|---|---|---|
| quad-depth heatmap | depth `ℓ` as colour ramp | every in-view quad above the screen floor is split (policy §0.1); below it, subdivision matches complexity + `ℓ < camera_depth + MAX_REL_DEPTH` (the sliding cap on supersampling depth only, `MAX_REL_DEPTH` ≥ the screen floor — R-88) |
