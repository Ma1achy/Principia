...
| **impurity mask** | per-sample `state` ≠ quad majority `state` | spatial mean = `outcome_impurity` — needed the `dominant_outcome` grain fix (pending change 1, resolved: one joint `class ⊕ detail` grain, `principia_dd_generation_root.md` §3.7); the field it reads is REVIEW_QUEUE RQ-18 |
