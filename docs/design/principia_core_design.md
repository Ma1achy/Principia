...
- (Amended twice: diagnostics ride the *forward* pass as tier-gated co-computations — **each sample's own Benettin FTLE shadow, and E ensemble copies that are full uniform samples** (the SSAA pool; sampling/SSAA note — the old "N ensemble shadows" framing is superseded). Each copy is the same kernel dispatched again with `copy_index` as a uniform — not a baked variant (R-102). "Separate kernels" applies only to different integration *protocols*, e.g. reversibility's forward+reverse — and these are build-time Rust variants, not runtime-authored.)
<!-- /retired-terms -->
