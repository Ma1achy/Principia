...
| `docs/archive/principia_spec_pending_changes.md` | the pending-changes register, closed: twelve changes; 1, 7, 9, 10, 11, 12 are folded into their owning files, 3 was folded earlier, 4 is moot; 2, 5, 6 and 8 are open decisions |
| `docs/archive/spec_sources/` | the retired LaTeX spec (`principia_spec_revised.tex`), the retired colour-map and COM-projection PDFs, the Burrau and render-quadtree PDFs, older IC Inspector copies, and `SECTION_MAP.md` (the LaTeX port ledger). Record only; the markdown is the authority |
