...
2. **Seed on the chart-parameter grid cell, not screen position.** The quad address `(depth, tx, ty)` is a cell of the slice plane's own frame (R-97), not a screen coordinate — so it is already navigation-stable (pan and zoom preserve cell identity). Seeding on it makes the sample pattern a property of *the render grid cell*, reproducible regardless of scroll position. When a quad splits, children are new addresses → fresh (correctly finer) patterns.
