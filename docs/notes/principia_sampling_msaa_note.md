...
2. **Data side — outcomes → spread metric.** The copies' classified outcomes reduce to a spread/agreement scalar (`ensemble_outcome_agreement`; see REVIEW_QUEUE RQ-16 for how it relates to the ledger's `spread_event`).
...
```
offset(k) = ( radical_inverse(2, k) , radical_inverse(3, k) )   # k = copy_index; first E points of Halton(2,3)
# optional decorrelation per cell (Cranley–Patterson rotation, deterministic):
offset = frac( offset(k) + integer_hash(depth, tx, ty, pixel_index_in_quad) )
```
...
- **quad address** — `(depth, tx, ty)`: the cell in the **current chart's normalised parameter square** `(s,t) ∈ [0,1]²`, at refinement depth. **No `chart_id` term** — a chart is *continuous* (`z₀, q₁, q₂, slices, links`); it cannot be hashed to an integer and does not belong in a per-pixel seed. The offset pattern is a property of *the grid cell being rendered*: stable under pan (cells keep identity), correctly view-relative under tilt (the cell's manifold *meaning* changes, but its sampling stays fixed — sampling is a rendering op on the current grid, not a manifold-absolute one). **NOT screen position** (the cell address is in chart-parameter space, which is already navigation-stable). Where "chart" *does* live: the **cache**, as a continuous validity signature (exact-match; change the chart → different ICs → invalidate) — the caching contract's identity/validity split. That is a separate mechanism from the offset seed; do not conflate them.
- **pixel_index_in_quad** — `(i, j)`, `0 ≤ i,j < SAMPLES_PER_QUAD_AXIS`: which nominal sample within the quad's sample grid.
- **copy_index** — `0 … E−1`: which of the E ensemble copies. The E offsets are the **first E points of the Halton (2,3) sequence** — *not* a hash-to-uniform and *not* a regular grid. Because they're a *sequence prefix*, **any E works** (you take the first E points, whatever E is — this is why E need not be a power of two). At the low sample counts Principia runs (`E+1 ≤ 16`), a low-discrepancy prefix has lower variance / better sub-pixel coverage than either random or a naive grid. Copy 0 = the un-jittered nominal (offset 0, i.e. the pixel centre); copies 1…E−1 are Halton points 1…E−1.

**Why this is the synthesis (beats both poles):**
- **Deterministic** → reproducible (same spec → same image) and **parity-clean without a shared RNG stream**: CPU and GPU both compute the same hash of the same integer coordinates. The earlier jitter-determinism-for-parity gap **closes for free** — there is no RNG *state* to synchronise; the "seed" is the coordinate both pipelines already hold.
...
- **Temporally stable → no shimmer.** The seed is *spatial* (quad+pixel+copy), never temporal — `t`/frame is excluded — so a static pixel gets the same offsets every frame. Spatial variety, temporal stability. (Frame-seeding would reintroduce shimmer; coordinate-seeding does not.)
...
**Cautions (this is where coordinate-seeded hashing bites):**
1. **Integer-domain, exact, for anything hashed.** The Halton points are fixed constants (identical CPU/GPU by construction). If the optional per-cell Cranley–Patterson rotation is used, hash the *integer* coordinates (quad ID, pixel `(i,j)`) with an integer bit-mixing hash (PCG / wang / xxhash-style) and convert to a fractional rotation only at the end — never hash floats (CPU/GPU float divergence before the hash breaks parity). The rotation is optional; without it, the same E-point prefix is reused per pixel (perfectly fine — it's still low-discrepancy and fixed).
2. **Seed on the chart-parameter grid cell, not screen position.** The quad address `(depth, tx, ty)` is a cell of the chart's `(s,t)` square, not a screen coordinate — so it is already navigation-stable (pan preserves cell identity). Seeding on it makes the sample pattern a property of *the render grid cell*, reproducible regardless of scroll position. When a quad splits, children are new addresses → fresh (correctly finer) patterns.

**Mental model:** quad address = *which cell of the chart's parameter grid*; pixel index = *which sample in that cell*; copy index = *which jittered neighbour of that sample* — the offsets are a fixed low-discrepancy Halton (2,3) prefix indexed by copy_index — deterministic and well-distributed, stable across time, pan, and CPU-vs-GPU; "Seeded random" in the sense that makes it a fixed pattern that looks random — which is what a research instrument wants.
...
All three are per-footprint reductions at the same stage; symbolic spread joins the spread family (a new `S_word` channel), aggregates into `QuadReduction`, and drives a **new refinement trigger** (split on high symbolic spread, additive to outcome-impurity and position-spread). The marginal *compute* cost is low — the copies' words are accumulated anyway; this retains and reduces them rather than discarding the copies'.

**Measure once built (empirical, like truncation rate):** whether symbolic spread is *additive* to outcome-impurity — does it fire earlier/differently (theory says yes), or in the same places (then it's not buying refinement value, though the word still earns its place as the core symbolic-dynamics feature). Build it per-copy, add the reduction + trigger, measure the marginal signal.
...
- **Sampling pattern (new decision):** offsets are a **fixed low-discrepancy Halton (2,3) prefix** indexed by `copy_index`, time excluded, **no chart_id** (chart is continuous → lives in the cache as a validity signature, not in the seed); optional deterministic per-cell rotation via `integer_hash(depth, tx, ty, pixel_index)`. Chosen over live-stochastic Monte-Carlo because lockstep marches the scene (no cross-frame accumulation; a new sample costs O(t) not O(1)) → Principia lives in the low-sample-count regime where a fixed well-distributed set wins. Closes the jitter-determinism-for-parity gap for free. Belongs in the render/scheduler contract as the sample-offset rule.
