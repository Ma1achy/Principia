> # ARCHIVED — NUMBERS VOID, METHOD REUSABLE
>
> The 68,685-leaf statistics here were computed on **the contaminated field**, with the α-exponent
> criterion and before the step-control fixes. Every field-conditional number is void.
>
> **Specifically void:** `tau` at the 0.4th percentile, the saturated structure fields,
> `frac_hot_between` at 45 distinct values (which is in any case an **arithmetic ceiling** — a
> fraction over `N² = 64` footprints has at most 65 values, and was misread as a signal property),
> the α distributions, and every selectivity/dynamic-range correlation.
>
> **What survives:** the `.prnq` parsing method, the statistical framing, and the practice of
> reporting stop-reason breakdowns rather than leaf counts. `principia_prnq_parser.py` still works.
>
> **Superseded by:** `principia_dd_refinement_policy.md`. Evidence: `prin-rs` `FINDINGS.md` §5,
> `results/tolerance_scaling/README.md`.
