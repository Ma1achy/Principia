...
Built and verified: `ic_inspector.html`, single file, hand-rolled HTML/canvas/JS (not Tweakpane), ships as a standalone artefact. Ports the canonical-frame decode + its inverse from `principia_spec.tex` §2 verbatim, CPU-side f64.
...
**Verification flags — need confirming against the spec:**
- **Momentum decode convention.** Implemented literally per §2.6: q → p_ρ=(q₀,q₁), p_λ=(q₂,q₃) with **no mass-unweighting**, unlike the position path (ρ = ρ̃/√μ_ρ). Confirm intended.
- **Chart constants.** μ_max, α_min, q_max defaulted to 4 / 0.05 / 2 — these set the representable range (when SAT / OUT_OF_CHART flags fire) and must match the real chart config.

**Open convention call.** BC landmarks move with mass — correct for the mass-weighted sphere, and matches the spec's "collision points depend on masses"; the dot lands exactly on a BC marker when two bodies coincide, at any mass ratio. If they should instead be *pinned*, that's the geometric fixed-landmark sphere — a different chart from the (α,β) decode. Undecided.
...
No live-march, no tile scheduler, no barrier. Single sample, one `SimState`. Cheap to stand up in the Tweakpane era.
...
- static, single-sample; reuses production encode / canonicalise / decode / conditioning paths verbatim — that reuse is *why* it's a valid test.
- Tweakpane for the numeric fields, a plain canvas for the drag layer. Fits the physics-dev GUI phase; the keyboard-nav / Solid.js rebuild inherits it later.
- everything here is CPU-side f64 single-sample — no shader work.

## Open questions

- name (still "IC Inspector" working title)
- shape sphere convention: keep **mass-weighted** (landmarks move with mass, matches decode) vs **geometric** (landmarks pinned) — see the convention call in the status block
