...
**The other suites, and the CI hardware (R-110).** Unit, property, numerical-gate and native golden suites run on every commit; benchmarks run on the human's own Mac via `prin profile`, at milestone gates and on demand, never on hosted runners (R-186); GUI screenshots run on GUI PRs and at the gates. GPU CI runs on GitHub-hosted runners (R-186): **lavapipe** on `ubuntu-latest`, and Metal on `macos-15` (Apple silicon, paravirtual Metal) for the Metal correctness suites only, both on every commit.
