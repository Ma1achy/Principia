...
- **Checkerboard-fraction ramp vs `dt`** — instead of a hard on/off at the ceiling, optionally *ramp* the stale fraction down as `dt` approaches the ceiling (fewer stale pixels when staleness is riskier). Whether a ramp is worth the complexity over a hard gate is a feel call, so it is made at the gate: the task captures both the hard gate and the ramp at the ceiling, and the human chooses at the M5 gate (✱ R-128).
