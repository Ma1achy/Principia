...
- **Navigation is chart construction:** pan/slice edit `z₀`, zoom/tilt edit the basis, the lock pins the centre — **all CPU-side edits to one uniform**; the GPU has no locked mode, no view mode, no second code path. Non-raw slice/tilt directions are tangents evaluated at the centre.
...
- **Lazy-in-time lockstep live-march.** Time is a **live clock**, not a stored dimension. A global **playhead** advances by fixed `dt`; every visible sample's `SimState` marches with it under a **barrier over the live set**; the fragment stage colours the current state. **No scrub, no stored history — state is O(1) in time.**
...
- **The struct:** hot `SimState` **136 B (FTLE on) / 88 B (FTLE off)**, 8-byte aligned; a **separate cold word buffer** (~16 B/sample, mixed-radix ~76-symbol free-group word, touched per branch-cut crossing + at resolve, never per-step); `ICDescriptor` (64 B); `QuadReduction` (~80 B) the summary. Descriptor: 8 bits used, 8–15 reserved.
- **Derive-at-read:** FTLE (`S_final/(n·dt)`, partial renorm finalised), ensemble spread (resolve-stage), the substep-log proxy (`countLeadingZeros` on the exact `total_substeps` u32), drift — all **derived at read, not stored fields**.
...
- **Deep zoom:** CPU (f64) owns global/nonlinear precision (quad centre/half-width, `x₀`, `J_D`); the GPU (f32) does only quad-local relative arithmetic (`x = x₀ + J_D·δ`). Screen-space floor is the everyday refinement stop (view-relative, not terminal); the true terminals are the linear-decoder `AT_F32_FLOOR` and the integration floor.
...
- **Keys & clocks:** **sim key** (change → re-integrate) · **render key** (change → recolour) · **playhead** (a live clock — advancing it is sim work; neither key) · **navigation** (re-addresses which quads are asked for; neither key). `render_scale` is render-side, **not** sim-key.
- **Two pipelines:** **kernel** = the compute side (Rust → SPIR-V); **shader** = the fragment side (WGSL). An **occupant** is a swappable slot fill (a `STEP` integrator on the compute side; a colour/brightness/combiner/post function on the fragment side). *(The vertical slice added a **second compute axis**: **regularisation** is an occupant slot independent of the stepper — `none` / AZ / **Heggie (default)** / logH. See `principia_integrator_contract.md` Part 2b.)*
...
11. **Branch determinism** — branch decisions bit-identical across backends via the comparison-only rule (frozen tables, integer counters, `d²`-comparisons; never runtime transcendentals); continuous values diverge freely. *[gpu_determinism, integrator dd]*
...
- **GUI design** — a formalisation session is pending; the corrected `gui_state_contract` (firewall = language boundary, scanned-fragment vs Rust-compute registry, state schema) is the clean foundation, but the full GUI *design* (Malachy's ideas) is unwritten.
- **The independent convergence reference** — *what* it is is now settled (§7: a separate double-double/arbitrary integrator for shared-bug immunity, distinct from the shared-kernel-at-f64 *precision* reference — validation_ground_truth §correctness-factoring, parity §1); its full *treatment* (protocol, when it runs, how it plugs into the harness) is the gap still to close.
