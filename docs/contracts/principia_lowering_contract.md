...
| **Shape sphere (α, β)** | derived-in-block × derived-in-block (config) | block inverse-free direct: (s,t)→(α,β) ranges | — | `system_image: DoubleCover` (2-to-1 over the φ hemispheres, R-104); residual none (both config DOF swept) |
