...
**Sim-key vs not (SETTLED):** `N`/`E`/`ftle`/`word` are **sim-key** (change *what's computed* → invalidate → re-boot, masked by the staleness backdrop). **`render_scale` is NOT sim-key and invalidates nothing** (payload purity — it moves the refinement target only; quality/device note, caching Part 2), which is exactly why the controller uses it as the continuous motion-time lever; `lock_to_native` and `MAX_REL_DEPTH` likewise invalidate nothing. Sim-key components move only at natural-invalidation moments (E excepted — copies drop/respawn live under motion gating without invalidating the nominal).
