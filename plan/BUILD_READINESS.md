...
*Checked 26 Sep 2026 against `main` at `86263a7` (after PR #11), with read-only `gh` calls. The build hasn't started:
...
| 5a | Self-hosted runner registered (R-174) | **waiting** | `gh api repos/Ma1achy/Principia/actions/runners` → `total_count: 0`. See HUMAN_SETUP §1. |
| 5b | Outside contributors need approval (R-174) | **waiting** | `…/actions/permissions/fork-pr-contributor-approval` → `approval_policy: first_time_contributors`. R-174 asks for approval of **all** outside collaborators (`all_external_contributors`). See HUMAN_SETUP §2. |
| 5c | `main` protected, requiring `ci` and `reviews-complete` (R-175, R-177) | **waiting** | `…/branches/main/protection` → 404 "Branch not protected"; `…/rulesets` → `[]`. The two checks can only be required once their workflows have run (after TASK-M0-01 and TASK-M0-03). See HUMAN_SETUP §3. |
...
**Waiting on the human:** 5a and 5b. For 5c, protection can be turned on now, and the two required checks added after
