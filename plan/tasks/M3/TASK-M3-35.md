...
## Deliverables
- `xtask golden patchwork` and `xtask golden stop-on-escape` over CPU payloads of the config slice (config_stability), the 'integrate everything' and 'off' images produced by `crates/validation`'s harness march.
...
- `cargo xtask golden stop-on-escape --propose` — per-pixel differences on the config slice (config_stability, R-166) measured for both experiments, the tolerance stated with a seam-structure check; the human confirms it at the M3 gate and it is recorded in decisions.md (REQ-EVT-023).
...
- *Was: "Gap: 'the config slice' / `config_stability` are not defined as chart + parameters in the corpus."* RQ-103 ruled: R-166 (a) — `config_stability` is in `fixtures/slices.toml`. "The config slice" here is `config_stability` by context: of the two saved Config-chart slices, the basin slice colours by terminal outcome and "the patchwork mechanism has no exposure here" (prin-rs `src/grid.rs:410-412`), which leaves the stability slice.
