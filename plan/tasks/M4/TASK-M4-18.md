...
- `xtask` gate `label-flip-report`: full-trajectory run on two GPU backends (Metal on GitHub-hosted `macos-15`, and lavapipe, R-110, R-186), the flip rate and, per flip, the stateless agreement of the branch operation; report header states the domain.
- CI: the parity job on every commit, on `macos-15` (Metal) and on lavapipe (R-110, R-186); the browser build (M8) depends on it.
