...
## Goal
Import picture (drop a Principia PNG or File › Import picture…) reads the pxpack, shows what is stored and what differs from now, and offers restore this view, only its stain, and open side by side. Saved views are pxpack snapshots with a thumbnail and "go", plus "Save this view", kept in the store §G9 defines (TASK-M8-08). Record a time sweep takes a time range, a frame count, the per-frame quality, a size at the view's aspect, a format (natively PNG frames, GIF, and MP4 through a system ffmpeg when present; in the browser zipped PNG frames, GIF via a wasm encoder, and MP4/WebM through WebCodecs where supported, R-131), overlays on or off, with pxpack in every frame, and integrates each frame to its own t through the blocking exporter — never by scrubbing.
...
- `cargo test -p engine record_frames_pxpack` — record 5 frames as PNG frames: each carries a pxpack whose time is its own frame time; size aspect equals the view's; natively MP4 is offered only when a system ffmpeg is found; in the browser MP4/WebM is offered only where WebCodecs supports it (R-131) (REQ-TOOL-106).
...
- RQ-108 ruled: R-131 — the formats offered per build (TASK-M8-30 builds the sinks).
