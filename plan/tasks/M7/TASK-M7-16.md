...
## Acceptance tests
- Review (code, perf): bake key = hash(colour source, colour uniforms); the preview widget samples the same GPU texture the render uses; rebake debounced ~120 ms and off the frame callback (REQ-RENDER-058).
