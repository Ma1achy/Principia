...
- **Milestone:** M2
- **Closes:** REQ-CHART-018, REQ-CHART-019, REQ-CHART-020, REQ-CHART-036, REQ-INT-003, REQ-VAL-023, REQ-CHART-048, REQ-CHART-049, REQ-CHART-053
...
- `cargo xtask gate shape-roundtrip` — Φ alone, without C (R-178): shape_vec(Φ(u, v)) == n(u, v) over random masses and shapes on both hemispheres, max |Δn| ≤ 1e−13 absolute, a provisional bound confirmed at the M2 gate (REQ-CHART-053, R-181); a deliberately wrong `atan2(−q, p)` inverse fails it (REQ-CHART-020).
- `cargo test -p kernel shape_sphere_canonical_fold` — Φ then C on lower-hemisphere points (w < 0) lands on the upper hemisphere at (θ, π − φ), and upper-hemisphere points are unchanged (R-178, R-141) (REQ-CHART-020).
...
- Proposal: the round-trip bound ≤ 1e−13 absolute, with the measured max |Δn| over the gate's samples as evidence; CI uses it provisionally until the human confirms it at the M2 gate (REQ-CHART-053, R-181, R-182).
