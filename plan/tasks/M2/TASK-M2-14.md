...
- `cargo test -p engine chart_descriptors` — each registered chart returns the descriptors; the shape sphere is n-to-1 with n = 2 (R-141); the full-range Burrau (Euclid) chart is DoubleCover (R-157) (REQ-CHART-014; the int (m, n) lattice's "bijective" is REQ-CHART-037's, M4).
...
## Notes
- RQ-71 ruled: R-141 — the shape sphere is n-to-1 (n = 2). RQ-127 ruled: R-157 — `DoubleCover` stays, as the full-range Burrau chart's value; REQ-CHART-014 is unblocked.
