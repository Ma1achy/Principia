...
## Deliverables
- `xtask/src/bench.rs` — `cargo xtask bench <bench>` and `cargo xtask bench --all` (not in the per-commit `cargo xtask ci`: nightly and at the gates, R-177), baselines in `fixtures/bench/<bench>/baseline.json`, compared with `prin profile diff`.
...
- `.github/workflows/nightly.yml` — `schedule` (daily) and `workflow_dispatch`: `cargo xtask bench --all`; the aggregate survey joins it at M8 (R-134, R-177).
- `.github/workflows/gate.yml` — `workflow_dispatch` with input `milestone`: runs `cargo xtask ci`, `cargo xtask bench --all` and `cargo xtask screenshot --all`, then `cargo xtask gate-report --milestone <Mn>`, which lists every requirement in that milestone's and every earlier milestone's gate block (`plan/MILESTONES.md`) with the result of its acceptance run, and uploads the report as an artifact (R-177).
...
- Review checklist (code): `ci.yml` does not run bench; `nightly.yml` is scheduled; `gate.yml` takes a `milestone` input and runs every suite (REQ-SYS-067).
