> # ARCHIVED — COMPLETE — this is the brief the whole slice was built from
>
> Delivered. The write-up is `prin-rs` `README.md` and `FINDINGS.md`; this brief is the specification it
> was built against and is kept as the record of what was asked for.
>
> **Read `FINDINGS.md` §9 and the closing section** — the defects found and the habit that found them
> are the more durable output.
>
...
**What to measure.** The claimed benefit is extending usable zoom from depth ~23 to ~50+. Report the
**divergence between direct and linearised against depth**, and where each hits its floor. Two
distinct limits: the linear-decoder `AT_F32_FLOOR`, and PR #11's plain-f64 cell-width floor at level
45.87. **The linearised path should not have the second**, because `delta` is quad-local and never
shrinks — that is the whole point, and it is the thing to verify rather than assume.
