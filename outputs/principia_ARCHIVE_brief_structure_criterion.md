> # ARCHIVED — SUPERSEDED — DO NOT IMPLEMENT
>
> This briefs the **alpha-exponent criterion and its slippy-map integration**. The criterion was
> **replaced** during the vertical slice; the alpha exponent split where halving the cell had *already*
> halved the spread — the **smooth** regions — and floored at discontinuities. Inverted.
>
> **Superseded by:** `principia_dd_refinement_policy.md` (`Policy::Tolerance`).
>
> **Do not implement §2/§3 (the structure measures, rank-vs-threshold, the two modes).** `tau_display`,
> `alpha_hi` and `k_frac` no longer exist as a parameter space; there is one knob, `eps`.
>
> **§4–4.6, the slippy map, LARGELY SURVIVES** and is still the design of record for camera integration:
> breadth-first, priority = camera relevance × structure, position in *priority* never in *veto*, the
> widened-margin destination model, coarse-ancestor fill, the persistent frontier. Read those sections.
>
> **§0's diagnosis is correct and worth keeping** — `tau_display` at the 0.4th percentile of its own
> distribution, the saturated structure fields. The remedy was wrong; the disease was real.
>
...
**No connected-components code exists in the repo.** Write a small union-find, with tests on
hand-built masks: single blob, two blobs, a one-cell-wide diagonal filament, full, empty.
...
`QuadTree` has **no neighbour lookup** — only `parent`/`children`/`sib_index`. Add
`neighbour(i, dir)` by descending from the root toward the neighbour centre and stopping at the
deepest computed node, which naturally returns the same-or-coarser neighbour. Test against a
geometric box-touch predicate over a whole tree, both directions, including across a level change.
**Computed at decision time, never stored on a `Quad`.**
...
> `error(B)` comes from the criterion-improvement plan and **may not be built yet**. If it is not,
> say so: the depth-variance test is then the *only* quantitative acceptance measure, which is
> thinner than this brief otherwise reads, and every §2.2 replace-vs-multiply comparison rests on it.
> Building `error(B)` first is a legitimate reordering if the comparison cannot otherwise be made.
