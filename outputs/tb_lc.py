...
def u_of_rho(rho, stable=True):
    """Inverse LC map (branch choice is irrelevant: rho(u) = rho(-u)).

    STABLE FORM (default). The naive u1 = sqrt((r + rho.x)/2) CATASTROPHICALLY CANCELS
    when rho points along -x, and u2 = rho.y/(2 u1) then amplifies the damage. This is not
    hypothetical: the Burrau default sits EXACTLY on that cut -- pair (1,2) separation is
    [3, 0], angle 0.0 deg, at t=0 -- and registration happens at every sync boundary, so the
    error is injected n_sync times per trajectory.

    Measured relative loss at 179.9 deg:  f64 3.5e-9 / f32 2.2e-2  (naive)
                                          f64 ~1e-16 / f32 5.96e-8 (stable, = f32 eps)

    A fixed cut also means accuracy depends on the ABSOLUTE ORIENTATION of a configuration.
    The physics is rotationally invariant; the naive implementation is not.

    Compute whichever component is larger and derive the other, so no subtraction of
    near-equal quantities occurs. `stable=False` restores the original form for
    reproducing results produced before this was found.
    """
    ...
    a = np.sqrt(np.maximum(0.5*(r + np.abs(rho[:, 0])), 0.0))   # never a cancellation
    safe = np.maximum(a, 1e-300)
    small = rho[:, 1]/(2.0*safe)
    posx = rho[:, 0] >= 0.0
    # rho.x >= 0: u1 = a (large), u2 = rho.y/(2a).
    # rho.x <  0: u2 = +/-a (large), u1 = rho.y/(2a) with the sign chosen so u1*u2 = rho.y/2.
    #   sign(rho.y) with rho.y == 0 must give +1, so use a >=0 test, not np.sign.
    sgn = np.where(rho[:, 1] >= 0.0, 1.0, -1.0)
    u1 = np.where(posx, a, np.abs(small))
    u2 = np.where(posx, small, sgn*a)
    u1 = np.where(a > 0.0, u1, 0.0)
    u2 = np.where(a > 0.0, u2, 0.0)
    return np.stack([u1, u2], axis=1)
