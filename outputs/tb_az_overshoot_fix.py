    ...
    for kk in range(n_sync):
        ...
        for a, b, c in TRIPLES:
            ...
            for _ in range(max_steps):
                ...
                if done.all():
                    break
                # OVERSHOOT FIX: shrink the final step so it lands exactly on dt_left.
                # dt = A*B*dtau, so the dtau that consumes the remaining time is
                # (dt_left - t_so_far)/(A*B). Take the smaller of that and the nominal dtau.
                A_ = np.maximum(np.einsum('sk,sk->s', s[:, 0:2], s[:, 0:2]), 1e-300)
                B_ = np.maximum(np.einsum('sk,sk->s', s[:, 4:6], s[:, 4:6]), 1e-300)
                rem = np.maximum(dt_left - s[:, 8], 0.0)
                dtau_eff = np.minimum(dtau, rem/(A_*B_))
                h = (dtau_eff*(~done)).astype(float)[:, None]
                k1 = sysm.deriv(s, E)
